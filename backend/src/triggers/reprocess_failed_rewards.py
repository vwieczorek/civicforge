...
LEASE_DURATION_SECONDS = int(
    os.environ.get("LEASE_DURATION_SECONDS", "300")
)  # 5 minutes
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"Starting failed rewards reprocessing at {datetime.now(timezone.utc).isoformat()}"
    )
    ...
    try:
        ...
        for item in items:
            ...
            if not acquire_lease(reward_id, current_time):
                logger.info(
                    f"Reward {reward_id} is being processed by another worker, skipping"
                )
                skipped += 1
                continue
...
def process_reward_with_lease(item: Dict[str, Any], current_time: datetime) -> str:
    ...
    logger.info(
        f"Processing reward {reward_id} for user {user_id}, retry attempt {retry_count + 1}"
    )
    ...
    if is_reward_already_processed(user_id, reward_id):
        logger.info(
            f"Reward {reward_id} already processed for user {user_id}, marking as resolved"
        )
...
def is_reward_already_processed(user_id: str, reward_id: str) -> bool:
    ...
    try:
        response = users_table.get_item(
            Key={"userId": user_id}, ProjectionExpression="processedRewardIds"
        )
...
def update_reward_status(reward_id: str, status: str, retry_count: int) -> None:
    ...
    try:
        update_expr = (
            "SET #status = :status, retryCount = :count, lastRetryAt = :timestamp"
        )
        expr_values = {
            ":status": status,
            ":count": retry_count,
