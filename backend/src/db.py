...
class DynamoDBClient:
    ...
    async def get_user(self, user_id: str) -> Optional[User]:
        ...
        try:
            async with self.get_resource() as dynamodb:
                logger.info(
                    f"Getting user {user_id} from table {self.users_table_name}"
                )
                response = await dynamodb.get_item(
                    TableName=self.users_table_name, Key={"userId": {"S": user_id}}
                )
            if "Item" in response:
    ...
    async def get_quest(self, quest_id: str) -> Optional[QuestDB]:
        ...
        try:
            async with self.get_resource() as dynamodb:
                response = await dynamodb.get_item(
                    TableName=self.quests_table_name, Key={"questId": {"S": quest_id}}
                )
            if "Item" in response:
                item = self._deserialize_item(response["Item"])
                return QuestDB(**item)
            ...
            for key, value in quest_dict.items():
                if isinstance(value, datetime):
                    ...
                    for attestation in value:
                        if "timestamp" in attestation and isinstance(
                            attestation["timestamp"], datetime
                        ):
    ...
    async def award_rewards(
        self, user_id: str, xp: int, reputation: int, quest_id: str = None
    ) -> None:
        ...
        try:
            ...
            if self.failed_rewards_table_name and quest_id:
                await self.track_failed_reward(
                    user_id, quest_id, xp, reputation, str(e)
                )
            raise
    ...
    async def track_failed_reward(
        self, user_id: str, quest_id: str, xp: int, reputation: int, error: str
    ) -> None:
        """Track failed reward for later retry"""
        if not self.failed_rewards_table_name:
            logger.warning(
                "Failed rewards table not configured, cannot track failed reward"
            )
            return
                    ...
                    if status:
                        ...
                        try:
                            ...
                            if "ResourceNotFoundException" in str(e):
                                ...
                                logger.warning(
                                    "StatusIndex GSI not found, falling back to scan"
                                )
    ...
    async def get_user_created_quests(
        self, user_id: str, limit: int = 20, start_key: Optional[Dict] = None
    ) -> Tuple[List[QuestDB], Optional[Dict]]:
            ...
            async with self.get_resource() as dynamodb:
                ...
                try:
                    ...
                    if "ResourceNotFoundException" in str(e):
                        ...
                        logger.warning(
                            "CreatorIndex GSI not found, falling back to scan"
                        )
    ...
    async def get_user_performed_quests(
        self, user_id: str, limit: int = 20, start_key: Optional[Dict] = None
    ) -> Tuple[List[QuestDB], Optional[Dict]]:
            ...
            async with self.get_resource() as dynamodb:
                ...
                try:
                    ...
                    if "ResourceNotFoundException" in str(e):
                        # GSI doesn't exist, fall back to scan with pagination
                        logger.warning(
                            "PerformerIndex GSI not found, falling back to scan"
                        )
    ...
    async def get_user_stats(self, user_id: str) -> Dict[str, int]:
        ...
        while True:
            quests, created_start_key = await self.get_user_created_quests(
                user_id, start_key=created_start_key
            )
        ...
        while True:
            quests, performed_start_key = await self.get_user_performed_quests(
                user_id, start_key=performed_start_key
            )
        ...
        completed_count = sum(
            1 for q in all_performed_quests if q.status == QuestStatus.COMPLETE
        )
    ...
    async def add_attestation_atomic(
        self, quest_id: str, attestation: Dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
                logger.warning(
                    f"Attestation already exists for {role} on quest {quest_id} or quest not in SUBMITTED state"
                )
                return False
    ...
    async def submit_quest_atomic(
        self, quest_id: str, performer_id: str, submission_text: str
    ) -> bool:
    ...
    async def dispute_quest_atomic(
        self, quest_id: str, user_id: str, reason: str
    ) -> bool:
