...
def lambda_handler(handler_func: Callable) -> Callable:
    ...
    def wrapper(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            # Add success metric
            metrics.add_metric(
                name="SuccessfulInvocation", unit=MetricUnit.Count, value=1
            )
            metrics.add_metric(
            ...
            metrics.add_metric(
                name="ErrorDuration", unit=MetricUnit.Milliseconds, value=duration_ms
            )
...
def log_api_request(method: str, path: str, status_code: int, duration_ms: float):
    ...
    metrics.add_metric(
        name="APIRequestDuration", unit=MetricUnit.Milliseconds, value=duration_ms
    )
