...
async def get_user_profile(
    ...
    created_quests_token: Optional[str] = Query(
        None, description="Pagination token for created quests"
    ),
    performed_quests_token: Optional[str] = Query(
        None, description="Pagination token for performed quests"
    ),
) -> Dict:
    ...
    if created_quests_token:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid created quests pagination token"
            )
    ...
    if performed_quests_token:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid performed quests pagination token"
            )

    # Get user's created quests with pagination
    created_quests, created_next_key = await db.get_user_created_quests(
        user_id, limit=limit, start_key=created_start_key
    )

    # Get user's performed quests with pagination
    performed_quests, performed_next_key = await db.get_user_performed_quests(
        user_id, limit=limit, start_key=performed_start_key
    )
    ...
    if created_next_key:
        profile_data["createdQuestsNextToken"] = base64.b64encode(
            json.dumps(created_next_key).encode()
        ).decode()

    if performed_next_key:
        profile_data["performedQuestsNextToken"] = base64.b64encode(
            json.dumps(performed_next_key).encode()
        ).decode()
...
async def update_wallet_address(
    ...
    if not wallet_address.startswith("0x") or len(wallet_address) != 42:
        raise HTTPException(
            status_code=400, detail="Invalid Ethereum wallet address format"
        )
    ...
    try:
        ...
        logger.error(
            f"Failed to update wallet address for user {user_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=500,
