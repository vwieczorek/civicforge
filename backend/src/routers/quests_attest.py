...
async def attest_quest(
    ...
    if not QuestStateMachine.can_user_attest(quest, user_id):
        ...
        if quest.status != QuestStatus.SUBMITTED:
            ...
            raise HTTPException(
                status_code=403, detail="Not authorized to attest this quest"
            )
    ...
    if QuestStateMachine.is_ready_for_completion(updated_quest):
        ...
        if was_completed:
            ...
            if updated_quest.performerId and feature_flags.is_enabled(
                FeatureFlag.REWARD_DISTRIBUTION, user_id=updated_quest.performerId
            ):
                try:
                    ...
                    logger.warning(
                        f"Quest {quest_id} completed but reward distribution failed: {e}"
                    )
                ...
                logger.info(
                    f"Quest {quest_id} completed but rewards disabled by feature flag"
                )
