...
async def claim_quest(
    ...
    if not QuestStateMachine.can_user_claim(quest, user_id):
        if quest.creatorId == user_id:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Quest is not available for claiming. Status: {quest.status}",
            )
...
async def submit_quest(
    ...
    if not QuestStateMachine.can_user_submit(quest, user_id):
        if quest.status != QuestStatus.CLAIMED:
            ...
            raise HTTPException(
                status_code=403, detail="Only the assigned performer can submit work"
            )
    ...
    success = await db.submit_quest_atomic(quest_id, user_id, sanitized_text)
    ...
    if not success:
        ...
        if updated_quest and updated_quest.status != QuestStatus.CLAIMED:
            ...
            raise HTTPException(
                status_code=403, detail="You are not authorized to submit this quest"
            )
...
async def dispute_quest(
    ...
    logger.info(
        f"Dispute initiated for quest {quest_id} by user {user_id}: {sanitized_reason}"
    )
