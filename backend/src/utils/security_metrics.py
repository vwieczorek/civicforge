...
metrics = Metrics(
    namespace=f"{os.environ.get('SERVICE_NAME', 'civicforge')}/{os.environ.get('STAGE', 'dev')}"
)


def record_authentication_failure(
    user_id: Optional[str] = None, reason: str = "unknown"
):
    ...
    logger.warning("authentication_failure", user_id=user_id, reason=reason)
...
def record_invalid_token_attempt(
    token_type: str = "unknown", error: Optional[str] = None
):
    ...
    logger.warning("invalid_token_attempt", token_type=token_type, error=error)
...
def record_suspicious_activity(activity_type: str, details: Dict[str, Any]):
    ...
    logger.warning("suspicious_activity", activity_type=activity_type, **details)
...
def record_security_event(event_type: str, severity: str, details: Dict[str, Any]):
    ...
    log_func("security_event", event_type=event_type, severity=severity, **details)
