...
def make_idempotent(
    ...
    return idempotent(config=custom_config, persistence_store=persistence_layer)


def make_idempotent_function(
    data_keyword_argument: str, expires_after_seconds: Optional[int] = None
):
    """
...
class IdempotencyHandler:
    ...
    def __init__(self, idempotency_key: str, expires_after_seconds: int = 3600):
    ...
    async def __aenter__(self):
        try:
            ...
            result = persistence_layer.get_record(idempotency_key=self.key)
            if result:
                self.is_cached = True
                self.cached_result = result
                logger.info("idempotency_cache_hit", idempotency_key=self.key)
        except IdempotencyItemAlreadyExistsError:
            self.is_cached = True
            self.cached_result = persistence_layer.get_record(idempotency_key=self.key)
        except IdempotencyAlreadyInProgressError:
            logger.warning("idempotency_in_progress", idempotency_key=self.key)
            raise
        except Exception as e:
            logger.error(
                "idempotency_check_failed", idempotency_key=self.key, error=str(e)
            )
        ...
        if not self.is_cached:
            ...
            try:
                persistence_layer.save_inprogress(
                    idempotency_key=self.key, expires_after_seconds=self.expires
                )
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        if exc_type and self._in_progress:
            ...
            try:
                persistence_layer.delete_record(idempotency_key=self.key)
            except Exception as e:
                logger.error(
                    "idempotency_cleanup_failed", idempotency_key=self.key, error=str(e)
                )
    ...
    def save_result(self, result: Any):
        ...
        if self._in_progress:
            try:
                ...
                logger.info("idempotency_result_saved", idempotency_key=self.key)
            except Exception as e:
                logger.error(
                    "idempotency_save_failed", idempotency_key=self.key, error=str(e)
                )
...
quest_creation_idempotent = make_idempotent(
    event_key_jmespath="body.questId", expires_after_seconds=86400  # 24 hours
)
