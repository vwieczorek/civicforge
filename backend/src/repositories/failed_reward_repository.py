...
class FailedRewardRepository(BaseRepository):
    ...
    async def get_pending_rewards(
        self, limit: int = 10, start_key: Optional[Dict] = None
    ...
    async def acquire_processing_lease(self, record_id: str, owner_id: str) -> bool:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
                self.logger.debug("lease_already_held", record_id=record_id)
                return False
    ...
    async def release_lease(self, record_id: str, owner_id: str) -> bool:
        ...
        try:
            ...
            self.logger.info("lease_released", record_id=record_id, owner_id=owner_id)
            return True

        except ClientError as e:
            if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
                self.logger.warning(
                    "lease_not_owned", record_id=record_id, owner_id=owner_id
                )
                return False
    ...
    async def mark_completed(self, record_id: str) -> bool:
        ...
        try:
            ...
            self.logger.info("reward_marked_completed", record_id=record_id)
            return True
