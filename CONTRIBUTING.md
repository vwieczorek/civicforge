...
- **Backend**: 85%+ coverage (currently at 85.43% ✅)
- **Frontend**: 70%+ coverage (currently at ~25% - deployment blocker)
...
For detailed backend testing guidelines, see [Testing Strategy](docs/reference/testing-strategy.md).
...
└── test_state_machine_*.py  # State transition tests
```

#### Best Practices

1. **Use Integration Tests with Moto Server Mode**
   ```python
   import pytest
   ...
   def moto_server():
       # Runs real AWS service simulation
       server = run_simple(...)
       yield server
   ```
...
2. **Test Realistic Scenarios**
   ```python
   def test_quest_completion_flow(api_client, test_user):
       # Create quest
       quest = api_client.post("/quests", json={...})
       
       # Claim quest
       api_client.post(f"/quests/{quest['id']}/claim")
       
       # Submit quest
       api_client.post(f"/quests/{quest['id']}/submit", 
                       json={"submissionText": "Done!"})
       
       # Verify state
       assert quest["status"] == "SUBMITTED"
   ```
...
3. **Avoid Over-Mocking**
   ...
   @patch('src.db.table')
   @patch('src.auth.verify')
   @patch('src.models.Quest')
   def test_something(mock1, mock2, mock3):
       pass
   
   # ✅ Good: Use real services where possible
   @mock_dynamodb
   def test_something(dynamodb_table):
       # Test with real DynamoDB behavior
       pass
   ```

### Writing Frontend Tests

For detailed frontend testing patterns and standards, see [Testing Strategy](docs/reference/testing-strategy.md).

#### Test Setup

All frontend tests use this stack:
- **Vitest**: Test runner
...
1. **Always Use MSW for API Mocking**
   ```typescript
   // src/mocks/handlers.ts
   export const handlers = [
     rest.get('/api/v1/quests/:id', (req, res, ctx) => {
       return res(ctx.json({ id: req.params.id, title: 'Test Quest' }))
     })
   ]
   ```
...
2. **Test User Interactions**
   ...
   it('should submit quest on button click', async () => {
...
2. **❌ Snapshot Tests Without Purpose**
   ```typescript
   // Bad: Brittle snapshot
   expect(component).toMatchSnapshot()
   
   // Good: Specific assertions
   expect(screen.getByRole('heading')).toHaveTextContent('Quest Title')
   ```

3. **❌ Manual Timer Manipulation**
   ```typescript
   // Bad: Using real timers
   setTimeout(() => {
     expect(something).toBe(true)
   }, 1000)
   
   // Good: Using testing utilities
   await waitFor(() => {
...
#### Deployment Gates
- **Backend**: Must maintain >70% coverage (currently 85.43% ✅)
- **Frontend**: Must achieve >70% coverage before deployment (currently ~25% ❌)
...
- Test error paths and unhappy flows
- Prioritize testing public APIs and user-facing features
